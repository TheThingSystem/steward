...
exports.register = function(manufacturers) {
  var device, devices, manufacturer, prop, props;

  for (manufacturer in manufacturers) {
    if (!manufacturers.hasOwnProperty(manufacturer)) continue;

    if (!deviceTypes[manufacturer]) deviceTypes[manufacturer] = {};

    props = manufacturers[manufacturer];
    for (prop in props) {
      if (!props.hasOwnProperty(prop)) continue;

      if (!deviceTypes[manufacturer][prop]) deviceTypes[manufacturer][prop] = {};

      devices = props[prop];
      for (device in devices) if (devices.hasOwnProperty(device)) deviceTypes[manufacturer][prop][device] = devices[device];
    }
  }
};
  ...
  noble.on('discover', function(peripheral) {
    ...
    peripheral_scan(peripheral, function(ble) {
      ...
      var value = function(s, c) {
        ...
        v = ble[s];
        if ((!v) || (!v.characteristics)) return '';
        v = v.characteristics[c];
        if ((!v) || (!v.value)) return '';
        v = v.value;
        u = v.toString('utf8');
      ...
      info.device = { url          : null
                    ...
                    , model        : { name        : modelName
                                     , description : value('180a', '2a24')
                                     , number      : value('180a', '2a24')
                                     }
                    , unit         : { serial      : value('180a', '2a25')
                                     , udn         : peripheral.uuid
                                     }
                    };
      ...
      logger.info('BLE ' + info.device.name, { uuid: peripheral.uuid,  id: value('180a', '2a23') });
...
var peripheral_scan = function(peripheral, callback) {
  ...
  peripheral.on('disconnect', function() {
    logger.info('BLE peripheralDisconnect', { localName: peripheral.advertisement.localName, uuid: peripheral.uuid });
  });

  peripheral.on('rssiUpdate', function(rssi) {
    logger.info('BLE peripheralRssiUpdate', { localName: peripheral.advertisement.localName, rssi: rssi });
  ...
  peripheral.on('servicesDiscover', function(services) {
    ...
    for (i = 0; i < services.length; i++) {
      ...
      m = uuidmap[s.uuid] || { name: null, type: '' };
