...
Cloud.prototype.scan = function(self) {
  ...
  self.cloudapi.getGarden(function(err, plants, sensors) {
    ...
    for (k in plants) {
      ...
      if (!plant.status.soil_mosture) plant.status.fertilizer = {};
      if (!plant.status.soil_mosture) plant.status.air_temperature = { instruction_key: 'temperature_good' };
      if (!plant.status.soil_mosture) plant.status.light = { instruction_key: 'light_good' };
